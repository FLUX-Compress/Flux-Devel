...
use flux_core_v1::{FluxCompressor, FluxDecompressor, FluxOptions, FluxCompressionLevel};
...
fn run_large_file_test(size_bytes: usize) {
    ...
    let d_stats = decompressor.decompress(&archive_path, temp_dest.path()).unwrap();
...
fn test_compress_1mb_high_entropy() {
    ...
    println!("\n[BENCH] 1MB high-entropy compression time: {:?}", duration);
    
    let mut decompressor = FluxDecompressor::new(options);
    let _d_stats = decompressor.decompress(&archive_path, temp_dest.path()).unwrap();

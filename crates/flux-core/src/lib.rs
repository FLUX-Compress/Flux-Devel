...
use archive::{ArchiveError, FileIndex, SolidBlockGrouper, DirectoryTree};
use buffer::circular::CircularBuffer;
use integrity::{MultiLevelIntegrity, ParallelCrc32, Crc32Hasher, Sha256Hasher};
...
impl FluxCompressor {
    ...
    pub fn compress_file(&mut self, input: &Path, output: &Path) -> Result<CompressionStats, ArchiveError> {
        reset_timings();
        ...
        tree.entries.push(archive::DirectoryEntry {
            ...
            entry_type: archive::EntryType::RegularFile { size: metadata.len() },
    ...
    pub fn compress_directory(&mut self, input: &Path, output: &Path) -> Result<CompressionStats, ArchiveError> {
        reset_timings();
        let total_start = std::time::Instant::now();
    ...
    fn compress_tree(&mut self, tree: DirectoryTree, output: &Path) -> Result<CompressionStats, ArchiveError> {
        ...
        let password_str = if self.options.password.is_null() {
            ...
            unsafe {
                std::ffi::CStr::from_ptr(self.options.password).to_str().ok()
            }
        };
        ...
        let (key, salt, sentinel) = if let Some(pass) = password_str {
            ...
            (None, crate::crypto::Salt([0u8; 16]), crate::crypto::stream::EncryptedChunk {
        ...
        let est_peak = window_size + target_cap + hash_table_size + hash_chains_size + 16 * 1024 * 1024;
        ...
        for entry in &tree.entries {
            if let archive::EntryType::RegularFile { size } = entry.entry_type {
                ...
                if let Some(ref cb) = self.progress_callback {
                    cb(bytes_processed, total_bytes, entry.path.to_string_lossy().into_owned());
                }

                let data = std::fs::read(&file_path).map_err(|e| ArchiveError::Io(e.to_string()))?;
        ...
        for builder in &mut builders {
            builder.entries_with_data.sort_by_key(|(entry, _)| entry.original_size);
        }
        ...
        file.write_all(&p_serialized).map_err(|e| ArchiveError::Io(e.to_string()))?;

        // Encrypted header placeholder (98 bytes if encrypted, 70 bytes if raw)
        let header_data_len = if is_encrypted { 98 } else { 70 };
        file.write_all(&vec![0u8; header_data_len]).map_err(|e| ArchiveError::Io(e.to_string()))?;
        ...
        let front_index_offset = file.stream_position().map_err(|e| ArchiveError::Io(e.to_string()))?;
        ...
        let write_start = std::time::Instant::now();
        file.write_all(&vec![0u8; front_index_size as usize]).map_err(|e| ArchiveError::Io(e.to_string()))?;
        add_write_time(write_start.elapsed().as_secs_f64());
        ...
        for builder in builders {
            ...
            flux_debug!("[FLUX] Processing solid block {}/{} (type={:?}, raw_size={} bytes)...", current_block_id + 1, total_blocks, block_type, raw_size);
            ...
            while chunk_index < uncompressed_data.len() {
                ...
                let (compressed_chunk_bytes, final_block_type, mut transform_stack, diag) = if (level == FluxCompressionLevel::Maximum || level == FluxCompressionLevel::Extreme) && block_type == archive::format::BlockType::Text {
                    ...
                    let (compressed_no_bwt, bt_no_bwt, diag_no_bwt) = compress_block(&transformed_no_bwt, block_type, level, &ts_no_bwt.media_filter_type);
                    ...
                    let (compressed_with_bwt, bt_with_bwt, diag_with_bwt) = compress_block(&transformed_with_bwt, block_type, level, &ts_with_bwt.media_filter_type);
                    ...
                    let (compressed, final_block_type, ts, diag) = if let Some(s) = stride {
                        if s % 4 == 0 && block_type == archive::format::BlockType::Multimedia {
                            ...
                            let (comp_generic, bt_generic, diag_generic) = compress_block(&transformed_generic, block_type, level, &ts_generic.media_filter_type);

                            // Try float channel split filter
                            let mut ts_float = TransformStack {
                                media_filter_applied: true,
                                media_filter_type: transform::MediaFilterType::FloatChannelSplit { channels: s / 4, mask: 0 },
                                ..Default::default()
                            };
                            ...
                            let (comp_float, bt_float, diag_float) = compress_block(&transformed_float, block_type, level, &ts_float.media_filter_type);
                            ...
                            let (compressed, bt, diag) = compress_block(&transformed_chunk_data, block_type, level, &ts.media_filter_type);
                        ...
                        let transformed_chunk_data = ts.apply(
                            ...
                            &match block_type {
                                ...
                                archive::format::BlockType::Multimedia => ContentType::Multimedia,
            ...
            let payload_ref = if has_key { &payload_to_write } else { &compressed_block_data };
            ...
            self.integrity.record_block(block_meta.block_id, payload_ref);
            ...
            let write_start = std::time::Instant::now();
            file.write_all(&block_header).map_err(|e| ArchiveError::Io(e.to_string()))?;
            file.write_all(payload_ref).map_err(|e| ArchiveError::Io(e.to_string()))?;
            add_write_time(write_start.elapsed().as_secs_f64());
        ...
        file.seek(SeekFrom::Start(front_index_offset)).map_err(|e| ArchiveError::Io(e.to_string()))?;
        file.write_all(&final_metadata_encrypted).map_err(|e| ArchiveError::Io(e.to_string()))?;

        // Seek to the end of the file to append the Back File Index
        file.seek(SeekFrom::End(0)).map_err(|e| ArchiveError::Io(e.to_string()))?;
        file.write_all(&final_metadata_encrypted).map_err(|e| ArchiveError::Io(e.to_string()))?;
        add_write_time(write_start.elapsed().as_secs_f64());

        // Compute correct physical body checksum by re-reading the written body sequentially
        let checksum_start = std::time::Instant::now();
        let body_end_offset = file.stream_position().map_err(|e| ArchiveError::Io(e.to_string()))?;
        file.seek(SeekFrom::Start(front_index_offset)).map_err(|e| ArchiveError::Io(e.to_string()))?;
        ...
        while remaining > 0 {
            ...
            file.read_exact(&mut read_buf[..to_read]).map_err(|e| ArchiveError::Io(e.to_string()))?;
        ...
        file.seek(SeekFrom::Start(body_end_offset)).map_err(|e| ArchiveError::Io(e.to_string()))?;
        // Write the trailing Archive Checksum (36 bytes: 4 bytes CRC32 + 32 bytes SHA-256)
        file.write_all(&archive_checksum.crc32.to_le_bytes()).map_err(|e| ArchiveError::Io(e.to_string()))?;
        file.write_all(&archive_checksum.sha256).map_err(|e| ArchiveError::Io(e.to_string()))?;
        add_write_time(write_start.elapsed().as_secs_f64());
        ...
        let write_start = std::time::Instant::now();
        file.seek(SeekFrom::Start(p_serialized.len() as u64)).map_err(|e| ArchiveError::Io(e.to_string()))?;
        file.write_all(&header_payload).map_err(|e| ArchiveError::Io(e.to_string()))?;
        add_write_time(write_start.elapsed().as_secs_f64());
        ...
        Ok(CompressionStats {
            ...
            compression_ratio: if original_total_size > 0 { final_size as f32 / original_total_size as f32 } else { 1.0 },
...
impl FluxDecompressor {
    ...
    pub fn read_archive_info(&mut self, input: &Path) -> Result<(u64, u64, u32, bool), ArchiveError> {
        let mut file = File::open(input).map_err(|e| ArchiveError::Io(e.to_string()))?;
        let file_len = file.metadata().map_err(|e| ArchiveError::Io(e.to_string()))?.len();

        let mut bootstrap_base = vec![0u8; 96];
        file.read_exact(&mut bootstrap_base).map_err(|e| ArchiveError::Io(e.to_string()))?;
        ...
        file.read_exact(&mut ct_bytes).map_err(|e| ArchiveError::Io(e.to_string()))?;
        bootstrap_full.extend_from_slice(&ct_bytes);

        let mut crc_bytes = [0u8; 4];
        file.read_exact(&mut crc_bytes).map_err(|e| ArchiveError::Io(e.to_string()))?;
        bootstrap_full.extend_from_slice(&crc_bytes);

        let plaintext_header = crate::crypto::header::PlaintextHeader::deserialize(&bootstrap_full)
        ...
        let key = if plaintext_header.is_encrypted {
            ...
            let k = crate::crypto::keys::KeyDerivation::derive_key(pass, &plaintext_header.salt)
                .map_err(|_| ArchiveError::WrongPassword)?;
            let matches = crate::crypto::sentinel::verify_sentinel(&k, &plaintext_header.sentinel_chunk)
                .map_err(|_| ArchiveError::DecryptionFailed)?;
            if !matches {
        ...
        file.read_exact(&mut header_data_bytes).map_err(|e| ArchiveError::Io(e.to_string()))?;

        let encrypted_header = if plaintext_header.is_encrypted {
            crate::crypto::header::decrypt_header_data(&header_data_bytes, &key.clone().unwrap())
                .map_err(|_| ArchiveError::DecryptionFailed)?
        } else {
        ...
        file.seek(SeekFrom::Start(archive_body_start)).map_err(|e| ArchiveError::Io(e.to_string()))?;
        let mut body_bytes = vec![0u8; (archive_body_end - archive_body_start) as usize];
        file.read_exact(&mut body_bytes).map_err(|e| ArchiveError::Io(e.to_string()))?;
        ...
        let (index, _) = FileIndex::deserialize_prefix(&decrypted_metadata)
            .map_err(|e| {
                flux_debug!("\n[READ_INFO FAIL] FileIndex::deserialize_prefix failed: {:?}", e);
                e
            })?;

        Ok((
            encrypted_header.original_total_size,
            encrypted_header.compressed_total_size,
            index.entries.len() as u32,
            plaintext_header.is_encrypted,
        ))
    }
    ...
    pub fn decompress(&mut self, input: &Path, output: &Path) -> Result<DecompressionStats, ArchiveError> {
        let start_time = std::time::Instant::now();
        let mut file = File::open(input).map_err(|e| ArchiveError::Io(e.to_string()))?;
        let file_len = file.metadata().map_err(|e| ArchiveError::Io(e.to_string()))?.len();

        // 1. Read and parse Plaintext Bootstrap Header
        let mut bootstrap_base = vec![0u8; 96];
        file.read_exact(&mut bootstrap_base).map_err(|e| ArchiveError::Io(e.to_string()))?;
        if &bootstrap_base[0..4] != b"FLUX" {
            return Err(ArchiveError::InvalidMagic);
        ...
        if test_alloc.try_reserve_exact(plaintext_header.window_size as usize).is_err() {
            return Err(ArchiveError::Io(format!(
                "This archive requires a {} MB decompression window, but only {} MB is available. Free memory or decompress on a machine with more RAM.",
                required_mb, available_mb
            )));
        ...
        // 2. Decrypt if encrypted & derive session key
        let password_str = if self.options.password.is_null() {
            None
        } else {
            unsafe {
        ...
        // 3. Read and parse Encrypted Header Data
        let header_data_len = if plaintext_header.is_encrypted { 98 } else { 70 };
        let mut header_data_bytes = vec![0u8; header_data_len];
        file.read_exact(&mut header_data_bytes).map_err(|e| ArchiveError::Io(e.to_string()))?;
        ...
        // 4. Verify Archive Integrity Checksums (36 trailing bytes)
        let archive_body_start = encrypted_header.file_index_offset;
        let archive_body_end = file_len - 36;
        ...
        file.read_exact(&mut trailing_crc_bytes).map_err(|e| ArchiveError::Io(e.to_string()))?;
        let trailing_crc = u32::from_le_bytes(trailing_crc_bytes);

        let mut trailing_sha = [0u8; 32];
        file.read_exact(&mut trailing_sha).map_err(|e| ArchiveError::Io(e.to_string()))?;

        let computed_cset = ChecksumSet::compute(&body_bytes);
        if computed_cset.sha256 != encrypted_header.archive_sha256 || computed_cset.sha256 != trailing_sha || computed_cset.crc32 != trailing_crc {
            flux_debug!("\n[DECOMPRESS FAIL] Checksum verification failed!");
            flux_debug!("[DEBUG DECOMPRESS] computed sha256: {:02x?}", computed_cset.sha256);
            flux_debug!("[DEBUG DECOMPRESS] header sha256:   {:02x?}", encrypted_header.archive_sha256);
            flux_debug!("[DEBUG DECOMPRESS] trailing sha256: {:02x?}", trailing_sha);
            flux_debug!("[DEBUG DECOMPRESS] computed crc32:  {:08x}", computed_cset.crc32);
            flux_debug!("[DEBUG DECOMPRESS] trailing crc32:  {:08x}", trailing_crc);
            panic!("Archive checksum validation failed!");
        }

        // 5. Read and parse Metadata Block (Index + Directory Tree)
        let metadata_payload_bytes = &body_bytes[..encrypted_header.file_index_size as usize];
        let decrypted_metadata = decrypt_payload(metadata_payload_bytes, &key)?;

        let (index, remaining_bytes) = FileIndex::deserialize_prefix(&decrypted_metadata)
            .map_err(|e| {
                flux_debug!("\n[DECOMPRESS FAIL] FileIndex::deserialize_prefix failed: {:?}", e);
                e
            })?;
        let tree = deserialize_directory_tree(remaining_bytes)
            .map_err(|e| {
                flux_debug!("\n[DECOMPRESS FAIL] deserialize_directory_tree failed: {:?}", e);
                ArchiveError::CorruptIndex
            })?;

        // Adjust tree base entries paths to align with index paths or reconstruct the full directory layout.
        // Reconstruct Directory Tree skeleton
        tree.restore(output)?;

        // 6. Extract solid blocks sequentially
        // The blocks payload on disk starts right after the Front File Index payload.
        let mut pos = encrypted_header.file_index_size as usize;
        let mut files_extracted = 0;
        let mut bytes_written = 0;

        let total_bytes: u64 = index.entries.iter().map(|e| e.original_size).sum();

        for block_id in 0..encrypted_header.solid_block_count {
            ...
            let uncompressed_size = u64::from_le_bytes(header_slice[13..21].try_into().unwrap()) as usize;
            ...
            let actual_block_hash = crate::integrity::ChecksumSet::compute(block_payload).sha256;
            if actual_block_hash != compressed_hash {
                return Err(ArchiveError::CorruptBlock(block_id));
            }

            // Decrypt block payload if encrypted
            let decrypted_payload = decrypt_payload(block_payload, &key)?;
            ...
            while sub_pos < decrypted_payload.len() {
                ...
                let sub_uncompressed_size = u32::from_le_bytes(decrypted_payload[sub_pos..sub_pos + 4].try_into().unwrap()) as usize;
                let sub_compressed_size = u32::from_le_bytes(decrypted_payload[sub_pos + 4..sub_pos + 8].try_into().unwrap()) as usize;
                let (sub_transform_stack, _) = TransformStack::deserialize(&decrypted_payload[sub_pos + 8..sub_pos + 21])
                    .map_err(|_| ArchiveError::CorruptBlock(block_id))?;
                flux_debug!(
                    "[DECOMPRESS DEBUG] block={}, sub_uncompressed_size={}, sub_compressed_size={}, sub_transform_stack={:?}",
                    block_id, sub_uncompressed_size, sub_compressed_size, sub_transform_stack
                );
                ...
                let decompressed_chunk = decompress_block(sub_chunk_bytes, block_type, lz77_uncompressed_size, &sub_transform_stack)?;
            ...
            let block_entries: Vec<&archive::format::FileEntry> = index.entries.iter()
            ...
            for entry in block_entries {
                ...
                if actual_cset.crc32 != entry.original_crc32 || actual_cset.sha256 != entry.original_sha256 {
                    return Err(ArchiveError::CorruptFile(entry.path.clone()));
                }
                ...
                std::fs::write(&file_out_path, file_data).map_err(|e| ArchiveError::Io(e.to_string()))?;
        ...
        // 7. Restore directory metadata (permissions and timestamps)
        tree.restore_metadata(output)?;
...
fn deserialize_frequency_table(data: &[u8]) -> Result<(crate::compress::rans::ProbabilityTable, &[u8]), ArchiveError> {
    if data.len() < 512 {
    ...
    let mut cumfreq = 0;
    for &f in &freq {
        symbols.push(crate::compress::rans::RansSymbol {
            freq: f,
            cumfreq,
        });
        cumfreq += f;
    ...
    Ok((crate::compress::rans::ProbabilityTable { symbols, alias_table }, &data[512..]))
}

fn compress_block(data: &[u8], block_type: archive::format::BlockType, level: FluxCompressionLevel, media_filter: &transform::MediaFilterType) -> (Vec<u8>, archive::format::BlockType, StreamDiagnostics) {
    ...
    if block_type == archive::format::BlockType::Raw || entropy > 7.95 {
        return (data.to_vec(), archive::format::BlockType::Raw, StreamDiagnostics::default());
    }

    match block_type {
        archive::format::BlockType::Raw => unreachable!(),
        archive::format::BlockType::Text | archive::format::BlockType::Binary | archive::format::BlockType::Mixed | archive::format::BlockType::Multimedia => {
            ...
            let (output, diag) = serialize_lz77_tokens(&tokens, media_filter, data, ppm_applied, ppm_arena_size);
...
fn decompress_block(compressed: &[u8], block_type: archive::format::BlockType, uncompressed_size: usize, transform_stack: &transform::TransformStack) -> Result<Vec<u8>, ArchiveError> {
    match block_type {
        ...
        archive::format::BlockType::Text | archive::format::BlockType::Binary | archive::format::BlockType::Mixed | archive::format::BlockType::Multimedia => {
            ...
            if decoded.len() != uncompressed_size {
                flux_debug!("[DECOMPRESS_BLOCK FAIL] decoded len {} != uncompressed_size {}", decoded.len(), uncompressed_size);
                return Err(ArchiveError::CorruptIndex);
            }
            Ok(decoded)
        }
...
fn rans_decode_stream(compressed: &[u8], len: usize, table: &crate::compress::rans::ProbabilityTable) -> Result<Vec<u8>, ArchiveError> {
    if compressed.len() < 8 {
        return Err(ArchiveError::CorruptIndex);
    }
...
impl StreamDiagnostics {
    fn merge(&mut self, other: &Self) {
        ...
        self.rep_indices_raw.extend_from_slice(&other.rep_indices_raw);
    }
}
...
fn print_diagnostics_report(diag: &StreamDiagnostics) {
    ...
    let flags_pct = if total_bytes > 0 { (diag.total_flags_bytes as f64 / total_bytes as f64) * 100.0 } else { 0.0 };
    let literals_pct = if total_bytes > 0 { (diag.total_literals_bytes as f64 / total_bytes as f64) * 100.0 } else { 0.0 };
    let lengths_pct = if total_bytes > 0 { (diag.total_lengths_bytes as f64 / total_bytes as f64) * 100.0 } else { 0.0 };
    let distance_pct = if total_bytes > 0 { ((diag.total_slots_bytes + diag.total_extra_bits_bytes) as f64 / total_bytes as f64) * 100.0 } else { 0.0 };
    let rep_indices_pct = if total_bytes > 0 { (diag.total_rep_indices_bytes as f64 / total_bytes as f64) * 100.0 } else { 0.0 };
    let tables_pct = if total_bytes > 0 { (diag.total_table_bytes as f64 / total_bytes as f64) * 100.0 } else { 0.0 };
    let headers_pct = if total_bytes > 0 { (diag.total_sub_block_headers_bytes as f64 / total_bytes as f64) * 100.0 } else { 0.0 };

    let avg_literals = if diag.num_sub_blocks > 0 { diag.total_literals_count as f64 / diag.num_sub_blocks as f64 } else { 0.0 };
    let avg_matches = if diag.num_sub_blocks > 0 { diag.total_matches_count as f64 / diag.num_sub_blocks as f64 } else { 0.0 };
    let avg_rep_matches = if diag.num_sub_blocks > 0 { diag.total_rep_matches_count as f64 / diag.num_sub_blocks as f64 } else { 0.0 };

    println!("   === FLUX STREAM ANALYSIS ===");
    println!("   Total compressed size:        {} bytes", total_bytes);
    println!("   Flags stream:        {} bytes ({:.2}%)", diag.total_flags_bytes, flags_pct);
    println!("   Literals stream:     {} bytes ({:.2}%)", diag.total_literals_bytes, literals_pct);
    println!("   Lengths stream:      {} bytes ({:.2}%)", diag.total_lengths_bytes, lengths_pct);
    println!("   Distance stream:     {} bytes ({:.2}%)", diag.total_slots_bytes + diag.total_extra_bits_bytes, distance_pct);
    println!("   Rep indices stream:  {} bytes ({:.2}%)", diag.total_rep_indices_bytes, rep_indices_pct);
    println!("   Frequency tables:    {} bytes ({:.2}%)   <- table overhead", diag.total_table_bytes, tables_pct);
    println!("   Sub-block headers:   {} bytes ({:.2}%)", diag.total_sub_block_headers_bytes, headers_pct);
    ...
    let dist_theoretical = ((diag.slots_raw.len() as f64 * dist_h0 / 8.0) + diag.total_extra_bits_bytes as f64).round() as usize;
    ...
    let lit_eff = if lit_actual > 0 { (lit_theoretical as f64 / lit_actual as f64) * 100.0 } else { 0.0 };
    let len_eff = if len_actual > 0 { (len_theoretical as f64 / len_actual as f64) * 100.0 } else { 0.0 };
    let dist_eff = if dist_actual > 0 { (dist_theoretical as f64 / dist_actual as f64) * 100.0 } else { 0.0 };
    let rep_eff = if rep_actual > 0 { (rep_theoretical as f64 / rep_actual as f64) * 100.0 } else { 0.0 };

    println!();
    println!("   Stream      Actual    Theoretical(order-0)   Efficiency");
    println!("   ──────────────────────────────────────────────────────");
    println!("   Literals    {:5} bytes   {:5} bytes                {:5.1}%", lit_actual, lit_theoretical, lit_eff);
    println!("   Lengths     {:5} bytes   {:5} bytes                {:5.1}%", len_actual, len_theoretical, len_eff);
    println!("   Distances   {:5} bytes   {:5} bytes                {:5.1}%", dist_actual, dist_theoretical, dist_eff);
    println!("   Rep Codes   {:5} bytes   {:5} bytes                {:5.1}%", rep_actual, rep_theoretical, rep_eff);
    println!();

    let lit_savings = if lit_h0 > 0.0 { (lit_h0 - lit_h1).max(0.0) / lit_h0 * 100.0 } else { 0.0 };
    println!("   Literals order-0 entropy: {:.4} bits/byte", lit_h0);
    println!("   Literals order-1 entropy: {:.4} bits/byte", lit_h1);
    println!("   Potential savings from order-1 modeling: {:.2}%", lit_savings);
    println!();

    let len_savings = if len_h0 > 0.0 { (len_h0 - len_h1).max(0.0) / len_h0 * 100.0 } else { 0.0 };
    println!("   Lengths order-0 entropy: {:.4} bits/byte", len_h0);
    println!("   Lengths order-1 entropy: {:.4} bits/byte", len_h1);
    println!("   Potential savings from order-1 modeling: {:.2}%", len_savings);
    println!();

    let dist_savings = if dist_h0 > 0.0 { (dist_h0 - dist_h1).max(0.0) / dist_h0 * 100.0 } else { 0.0 };
    println!("   Distances order-0 entropy: {:.4} bits/byte", dist_h0);
    println!("   Distances order-1 entropy: {:.4} bits/byte", dist_h1);
    println!("   Potential savings from order-1 modeling: {:.2}%", dist_savings);
    println!();

    let rep_savings = if rep_h0 > 0.0 { (rep_h0 - rep_h1).max(0.0) / rep_h0 * 100.0 } else { 0.0 };
    println!("   Rep Codes order-0 entropy: {:.4} bits/byte", rep_h0);
    println!("   Rep Codes order-1 entropy: {:.4} bits/byte", rep_h1);
    println!("   Potential savings from order-1 modeling: {:.2}%", rep_savings);
    println!("   ============================");
}
...
fn serialize_lz77_tokens(tokens: &[Lz77Token], media_filter: &transform::MediaFilterType, data: &[u8], ppm_applied: bool, ppm_arena_size: usize) -> (Vec<u8>, StreamDiagnostics) {
    ...
    for token in tokens {
        match token {
            ...
            Lz77Token::Match { distance, length } => {
                ...
                let (slot, extra_val, extra_bits) = crate::compress::lz77::distance_to_slot(*distance);
...
fn deserialize_lz77_tokens(data: &[u8], transform_stack: &transform::TransformStack, uncompressed_size: usize) -> Result<Vec<Lz77Token>, ArchiveError> {
    if data.len() < 24 {
    ...
    let expected_table_overhead = if transform_stack.ppm_applied { 2048 } else { 2560 };
    ...
    if data.len() < pos_offset + flags_encoded_len + literals_encoded_len + lengths_encoded_len + distance_slots_encoded_len + rep_indices_encoded_len {
        flux_debug!(
    ...
    let flags_stream = rans_decode_stream(flags_encoded, num_flag_bytes, &flags_table)
        .map_err(|e| {
            flux_debug!("[LZ77 DECODE FAIL] rans_decode_stream flags failed: {:?}", e);
            e
        })?;
    ...
    let lengths_stream = rans_decode_stream(lengths_encoded, num_matches + num_rep_matches, &lengths_table)
        .map_err(|e| {
            flux_debug!("[LZ77 DECODE FAIL] rans_decode_stream lengths failed: {:?}", e);
            e
        })?;
    let slots_stream = rans_decode_stream(slots_encoded, num_matches, &slots_table)
        .map_err(|e| {
            flux_debug!("[LZ77 DECODE FAIL] rans_decode_stream slots failed: {:?}", e);
            e
        })?;
    let rep_indices_stream = rans_decode_stream(rep_indices_encoded, num_rep_matches, &rep_indices_table)
        .map_err(|e| {
            flux_debug!("[LZ77 DECODE FAIL] rans_decode_stream rep_indices failed: {:?}", e);
            e
        })?;
    ...
    let min_match_map = crate::compress::lz77::build_min_match_map(&transform_stack.media_filter_type, uncompressed_size);
    let mut pos = 0;
    ...
    let mut literals_decoder = if transform_stack.ppm_applied {
        if literals_encoded.is_empty() {
            ...
            Some(crate::compress::rans::RansDecoder::new(literals_encoded.to_vec()))
        }
    } else {
        None
    };
    let mut ppm = if transform_stack.ppm_applied {
        Some(crate::compress::ppm::PpmModel::new(transform_stack.ppm_arena_size))
    } else {
        None
    };
        ...
        match flag {
            0 => {
                let b = if transform_stack.ppm_applied {
                    let dec = literals_decoder.as_mut().ok_or(ArchiveError::CorruptIndex)?;
...
fn deserialize_encrypted_chunks(data: &[u8]) -> Result<(Vec<crate::crypto::stream::EncryptedChunk>, &[u8]), String> {
    if data.len() < 4 {
...
fn encrypt_payload(data: &[u8], key: &Option<crate::crypto::EncryptionKey>) -> Result<Vec<u8>, ArchiveError> {
    if let Some(k) = key {
        let mut encryptor = crate::crypto::stream::StreamEncryptor::new(k.clone());
        let chunks = encryptor.encrypt_stream(data)
            .map_err(|e| ArchiveError::Io(e.to_string()))?;
...
fn decrypt_payload(data: &[u8], key: &Option<crate::crypto::EncryptionKey>) -> Result<Vec<u8>, ArchiveError> {
    if let Some(k) = key {
        let mut decryptor = crate::crypto::stream::StreamDecryptor::new(k.clone());
        let (chunks, _) = deserialize_encrypted_chunks(data)
            .map_err(|_| ArchiveError::DecryptionFailed)?;
        let decrypted = decryptor.decrypt_stream(&chunks)
            .map_err(|_| ArchiveError::DecryptionFailed)?;
        Ok(decrypted)
    } else {
        Ok(data.to_vec())
...
fn deserialize_directory_tree(data: &[u8]) -> Result<crate::archive::DirectoryTree, String> {
    ...
    let root_str = String::from_utf8(data[4..4 + root_len].to_vec())
        .map_err(|e| e.to_string())?;
    let root = PathBuf::from(root_str);
    
    let count = u32::from_le_bytes(data[4 + root_len..4 + root_len + 4].try_into().unwrap()) as usize;
        ...
        let entry_type = match tag {
            ...
            2 => {
                ...
                let target_str = String::from_utf8(data[pos..pos + target_len].to_vec())
                    .map_err(|e| e.to_string())?;
                pos += target_len;
                crate::archive::EntryType::Symlink { target: PathBuf::from(target_str) }
            }
    ...
    Ok(crate::archive::DirectoryTree {
        root,
        entries,
    })
}
...
mod tests {
    ...
    fn test_compress_decompress_directory() {
        ...
        let c_stats = compressor.compress_directory(src_path, &archive_path).unwrap();
    ...
    fn get_bwt_applied_flags(archive_path: &Path) -> Vec<bool> {
        ...
        let plaintext_header = crate::crypto::header::PlaintextHeader::deserialize(&bootstrap_full).unwrap();
        ...
        let encrypted_header = crate::crypto::header::EncryptedHeaderData::deserialize(&header_data_bytes).unwrap();
        ...
        for _block_id in 0..encrypted_header.solid_block_count {
            ...
            while sub_pos < block_payload.len() {
                // Read sub-block header (21 bytes)
                let _sub_uncompressed_size = u32::from_le_bytes(block_payload[sub_pos..sub_pos + 4].try_into().unwrap()) as usize;
                let sub_compressed_size = u32::from_le_bytes(block_payload[sub_pos + 4..sub_pos + 8].try_into().unwrap()) as usize;
    ...
    fn test_adaptive_bwt_decision() {
        ...
        for len in (1000..50000).step_by(1000) {
            ...
            let mut ts_no_bwt = TransformStack::default();
            let trans_no_bwt = ts_no_bwt.apply(candidate.clone(), 3.0, &ContentType::Text, None, false);
            let (comp_no_bwt, _, _) = compress_block(&trans_no_bwt, archive::format::BlockType::Text, FluxCompressionLevel::Maximum, &transform::MediaFilterType::None);

            let mut ts_with_bwt = TransformStack::default();
            let trans_with_bwt = ts_with_bwt.apply(candidate.clone(), 3.0, &ContentType::Text, None, true);
            let (comp_with_bwt, _, _) = compress_block(&trans_with_bwt, archive::format::BlockType::Text, FluxCompressionLevel::Maximum, &transform::MediaFilterType::None);

            if comp_with_bwt.len() < comp_no_bwt.len() {
                bwt_friendly_data = candidate;
                found_friendly = true;
                println!("[TEST] Found friendly size: {}, no-BWT: {}, with-BWT: {}", len, comp_no_bwt.len(), comp_with_bwt.len());
                break;
            }
        }

        assert!(found_friendly, "Failed to find a BWT-friendly text size where BWT helps");
        ...
        for len in (1000..50000).step_by(1000) {
            ...
            if comp_with_bwt.len() >= comp_no_bwt.len() {
                ...
                println!("[TEST] Found unfriendly size: {}, no-BWT: {}, with-BWT: {}", len, comp_no_bwt.len(), comp_with_bwt.len());
                break;
            }
        }
        
        assert!(found_unfriendly, "Failed to find a BWT-unfriendly text size where BWT does not help");
        ...
        compressor.compress_file(&path_friendly, &archive_friendly).unwrap();

        // Compress unfriendly with Maximum mode
        let archive_unfriendly = src_path.join("unfriendly.flx");
        compressor.compress_file(&path_unfriendly, &archive_unfriendly).unwrap();
        ...
        assert!(!friendly_flags.is_empty(), "Friendly archive should have sub-blocks");
        assert!(!unfriendly_flags.is_empty(), "Unfriendly archive should have sub-blocks");

        // The friendly text should keep BWT
        assert!(friendly_flags.iter().any(|&f| f), "BWT should be applied to BWT-friendly text");

        // The unfriendly text should skip BWT
        assert!(unfriendly_flags.iter().all(|&f| !f), "BWT should be skipped for BWT-unfriendly text");
    }
    ...
    fn test_stream_separated_roundtrip() {
        let test_cases = vec![
            ...
            // Case 3: Matches only
            vec![
                Lz77Token::Match { distance: 1, length: 10 },
                Lz77Token::Match { distance: 300, length: 258 },
                Lz77Token::Match { distance: 32768, length: 4 },
            ],
            // Case 4: Mixed
            vec![
                Lz77Token::Literal(b'a'),
                Lz77Token::Match { distance: 10, length: 15 },
                Lz77Token::Literal(b'b'),
                Lz77Token::Match { distance: 5, length: 5 },
                Lz77Token::Literal(b'c'),
                Lz77Token::Match { distance: 30000, length: 100 },
            ],
        ];

        let ts = transform::TransformStack::default();
        for tokens in test_cases {
            let (serialized, _) = serialize_lz77_tokens(&tokens, &ts.media_filter_type, &[0u8; 1000], false, 0);
    ...
    fn test_coord_mismatch() {
        ...
        let mut ts = TransformStack {
            media_filter_applied: true,
            media_filter_type: transform::MediaFilterType::FloatChannelSplit { channels: 3, mask: 0 },
            ..Default::default()
        };
        ...
        let (serialized, _) = serialize_lz77_tokens(&tokens, &ts.media_filter_type, &transformed, false, 0);
        let deserialized_tokens = deserialize_lz77_tokens(&serialized, &ts, transformed.len()).unwrap();
        ...
        while chunk_index < data.len() {
            ...
            let (compressed_chunk, ts) = if let Some(s) = stride {
                if s % 4 == 0 {
                    // Try generic transpose
                    let mut ts_generic = TransformStack::default();
                    let transformed_generic = ts_generic.apply(chunk_data.clone(), 3.0, &ContentType::Multimedia, stride, false);
                    let (comp_generic, _, _) = compress_block(&transformed_generic, crate::archive::format::BlockType::Multimedia, crate::FluxCompressionLevel::Balanced, &ts_generic.media_filter_type);

                    // Try float channel split
                    let mut ts_float = TransformStack {
                        media_filter_applied: true,
                        media_filter_type: crate::transform::MediaFilterType::FloatChannelSplit { channels: s / 4, mask: 0 },
                        ..Default::default()
                    };
                    let transformed_float = ts_float.apply(chunk_data.clone(), 3.0, &ContentType::Multimedia, None, false);
                    let (comp_float, _, _) = compress_block(&transformed_float, crate::archive::format::BlockType::Multimedia, crate::FluxCompressionLevel::Balanced, &ts_float.media_filter_type);
                    ...
                    let transformed_normal = ts_normal.apply(chunk_data.clone(), 3.0, &ContentType::Multimedia, stride, false);
                    let (comp_normal, _, _) = compress_block(&transformed_normal, crate::archive::format::BlockType::Multimedia, crate::FluxCompressionLevel::Balanced, &ts_normal.media_filter_type);
            ...
            // Decompress chunk
            let decompressed_tokens = deserialize_lz77_tokens(&compressed_chunk, &ts, chunk_data.len());
            ...
            if decoded.len() != chunk_data.len() {
                println!("Chunk {} failed: decoded.len() = {}, expected = {}, stride = {:?}, ts = {:?}", chunk_id, decoded.len(), chunk_data.len(), stride, ts);
    ...
    fn test_multi_level_roundtrips() {
        ...
        for &level in &levels {
            ...
            let mut decompressor = FluxDecompressor::new(options);
            let (orig_sz, _comp_sz, file_count, encrypted) = decompressor.read_archive_info(&archive_path).unwrap();
    ...
    fn test_bwt_ppm_roundtrip() {
        ...
        let (compressed, _) = serialize_lz77_tokens(&tokens, &transform::MediaFilterType::None, &transformed, true, ts.ppm_arena_size);
        
        let decompressed = decompress_block(&compressed, archive::format::BlockType::Text, transformed.len(), &ts).unwrap();
    ...
    fn test_diagnose_corpus_block() {
        ...
        // Find mod.rs entry
        let mod_entry = index.entries.iter().find(|e| e.path.contains("components/mod.rs") || e.path.contains("components\\mod.rs")).unwrap().clone();
        ...
        let mut block_entries: Vec<&crate::archive::format::FileEntry> = index.entries.iter()
        ...
        for entry in &block_entries {
            let path = workspace_root.join("scratch/real_world_corpus").join(&entry.path);
        ...
        println!("Target block type: {:?}, compressed size: {}, original size: {}", block_type, target_payload.len(), target_uncompressed_size);
        ...
        while sub_pos < decrypted_payload.len() {
            let sub_uncompressed_size = u32::from_le_bytes(decrypted_payload[sub_pos..sub_pos + 4].try_into().unwrap()) as usize;
            let sub_compressed_size = u32::from_le_bytes(decrypted_payload[sub_pos + 4..sub_pos + 8].try_into().unwrap()) as usize;
            let (sub_transform_stack, _) = crate::transform::TransformStack::deserialize(&decrypted_payload[sub_pos + 8..sub_pos + 21]).unwrap();
            println!("Sub-block {}: uncompressed_size={}, compressed_size={}, ts={:?}", chunk_id, sub_uncompressed_size, sub_compressed_size, sub_transform_stack);
            sub_pos += 21;
            ...
            let decompressed_chunk = decompress_block(sub_chunk_bytes, block_type, lz77_uncompressed_size, &sub_transform_stack).unwrap();
            let restored_chunk = sub_transform_stack.reverse(decompressed_chunk);
            restored_payload.extend_from_slice(&restored_chunk);
            chunk_id += 1;
        }

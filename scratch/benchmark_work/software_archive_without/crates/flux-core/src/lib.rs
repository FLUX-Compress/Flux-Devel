...
impl FluxCompressor {
    ...
    fn compress_tree(
        ...
        for (idx, entry) in tree.entries.iter().enumerate() {
            if let Some(cls) = classifications.get(&(idx as u64)) {
                path_to_content_type.insert(entry.path.to_string_lossy().into_owned(), cls.content_type);
            }
        }
                ...
                let (compressed_chunk_bytes, final_block_type, mut transform_stack, diag) =
                        ...
                        && block_type == archive::format::BlockType::Text
                    {
                        ...
                        let (compressed, final_block_type, ts, diag) = if let Some(s) = stride {
                            ...
                            let mut chunk_content_type = ContentType::Binary;
                            for file in &block_files {
                                let file_start = file.block_offset as usize;
                                let file_end = file_start + file.original_size as usize;
                                if chunk_index >= file_start && chunk_index < file_end {
                                    if let Some(ct) = path_to_content_type.get(&file.path) {
                                        chunk_content_type = *ct;
                                    }
                                    break;
                                }
                            }

                            let is_exec = chunk_content_type == ContentType::Executable
                                || crate::analysis::executable_detect::is_pe_x86_or_x64(&chunk_data)
                                || crate::analysis::executable_detect::is_elf_x86_or_x64(&chunk_data)
                                || crate::analysis::executable_detect::is_mach_o_x86_or_x64(&chunk_data);

                            if is_exec && crate::analysis::executable_detect::should_apply_bcj_x86(&chunk_data) {
                                bcj_applied = true;
                                let mut ts = TransformStack {
                                    media_filter_applied: true,
                                    media_filter_type: transform::MediaFilterType::BcjX86,
                                    ..Default::default()
                                };
                                let transformed_chunk_data = ts.apply(
                                    chunk_data,
    ...
    fn compress_tree_split(
        ...
        for builder in &builders {
            for (entry, data) in &builder.entries_with_data {
                if crate::analysis::executable_detect::is_pe_x86_or_x64(data)
                    || crate::analysis::executable_detect::is_elf_x86_or_x64(data)
                    || crate::analysis::executable_detect::is_mach_o_x86_or_x64(data)
                {
                    executable_paths.insert(entry.path.clone());
                }
            }
        }
                ...
                let (compressed_chunk_bytes, final_block_type, mut transform_stack, diag) =
                        ...
                        && block_type == archive::format::BlockType::Text
                    {
                        ...
                        let (compressed, final_block_type, ts, diag) = if let Some(s) = stride {
                            ...
                            let mut chunk_is_exec = false;
                            for file in &block_files {
                                let file_start = file.block_offset as usize;
                                let file_end = file_start + file.original_size as usize;
                                if chunk_index >= file_start && chunk_index < file_end {
                                    if executable_paths.contains(&file.path) {
                                        chunk_is_exec = true;
                                    }
                                    break;
                                }
        ...
        if n == 1 {
            ...
            file_1
                ...
                .map_err(|e| ArchiveError::Io(e.to_string()))?;
            file_1
                .write_all(&plaintext_header_final.serialize())
                .map_err(|e| ArchiveError::Io(e.to_string()))?;
